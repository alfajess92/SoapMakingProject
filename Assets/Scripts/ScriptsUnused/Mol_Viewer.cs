...
public class Mol_Viewer : MonoBehaviour
{
    ...
    //            if (res.GetResidueProperty(9))
    //            { //water
    //                tag = "water";
    //            }
    //            else if (res.GetAtomProperty(atom, 4) || (atom.IsHydrogen() && !res.GetResidueProperty(5)))
    //            { //ligand
    //                if (res.GetResidueProperty(5))
    //                { //ion (should be 3 but a bug in openbabel labels ions as protein, thats why the check has to be done inside the ligand check)
    //                    tag = "ion";
    //                }
    //                else
    //                {
    ...
    //                if (res.GetAtomProperty(atom, 0))
    //                { //alpha carbon
    //                    alphaCarbons[alphaCarbonCount] = atom;
    //                    if (alphaCarbonCount > 6)
    //                    { //check if the ribbon is alpha helix using torsion angles
    ...
    //                    if (proteinText)
    //                    { //only displays text on alpha carbons
    ...
    //					{
    //						if (col is SphereCollider && col.transform.position != centerPosition)
    //						{ //skips it's own collider
    ...
    //    if (renderPharmacophore)
    //    { //creates a pharmacophore sphere
    //        if (renderEXC)
    //        {
    ...
    //		for (int i = 0; i < renderers.Length; i++)
    //		{ // different amount of  bonds depending on order
    //			if (i % 2 == 0)
    ...
    //			showMode("hetatmbs");
    //		}
    //		else if (PlayerPrefs.GetString("hetatmL") != "True" && !hetatmStick)
    //		{ //ligand is hidden
    ...
    //		if (tag == "hbond")
    //		{ //does not have a renderer and needs to be handled seperatly
    //			HBondShowing = toggle;
    ...
    //	{
    //		if (Vector3.Distance(position1, position2) < 3.5f)
    //		{ //we have a hBond
    ...
    //		{ //if two atoms are selected the distance between them is displayed
